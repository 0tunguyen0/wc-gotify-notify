...
class WC_Gotify_Support_Sidebar {
	 ...
	 */
	public static function clear_cache( string $cache_key = 'wc_gotify_sidebar_data' ): void {
	 ...
	 * @return array|false
	 */
	public function get_ad_data(): array|false {
	...
	public function display(): void {
		echo $this->get_html(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- All content is escaped within get_html().
	}
			...
			<?php if ( ! empty( $banners ) ) : ?>
				<div id="<?php echo esc_attr( $slider_id ); ?>" class="postbox wc-gotify-sb-slider">
					...
					<?php if ( count( $banners ) > 1 ) : ?>
						...
						<?php
						// Enqueue slider JS and pass configuration.
						wp_enqueue_script(
							'wc-gotify-slider',
							WC_GOTIFY_URL . 'assets/js/slider.js',
							array(),
							WC_GOTIFY_VERSION,
							true
						);
						wp_add_inline_script(
							'wc-gotify-slider',
							'window.wcGotifySlider = ' . wp_json_encode( array(
								'sliderId'      => $slider_id,
								'slideInterval'  => (int) $this->args['slide_interval'],
							) ) . ';',
							'before'
						);
						?>
