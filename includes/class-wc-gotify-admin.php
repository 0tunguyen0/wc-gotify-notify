...
final class WC_Gotify_Admin {
	 ...
	 */
	private static ?WC_Gotify_Admin $instance = null;
	 ...
	 */
	public static function instance(): WC_Gotify_Admin {
	...
	public function enqueue_admin_scripts( string $hook_suffix ): void {
		if ( 'toplevel_page_wc-gotify' !== $hook_suffix ) {
			return;
		}
	...
	public function sanitize_settings( $input ): array {
		...
		$output = array(
			'server_url'      => esc_url_raw( trim( $input['server_url'] ?? '' ) ),
			'app_token'       => sanitize_text_field( trim( $input['app_token'] ?? '' ) ),
			'priority'        => max( 0, min( 10, (int) ( $input['priority'] ?? 5 ) ) ),
			'title'           => sanitize_text_field( $input['title'] ?? WC_Gotify_Core::default_settings()['title'] ),
			'message'         => sanitize_textarea_field( $input['message'] ?? WC_Gotify_Core::default_settings()['message'] ),
			'async'           => empty( $input['async'] ) ? 0 : 1,
			'basic_auth_user' => sanitize_text_field( $input['basic_auth_user'] ?? '' ),
			'basic_auth_pass' => (string) ( $input['basic_auth_pass'] ?? '' ),
		);
	...
	/**
	 * Render the Gotify Server URL field.
	 */
	public function render_field_server_url(): void {
	...
	/**
	 * Render the Application Token field.
	 */
	public function render_field_app_token(): void {
		$s = WC_Gotify_Core::instance()->get_settings();
		printf(
	 ...
	 */
	public function render_field_priority(): void {
		$s = WC_Gotify_Core::instance()->get_settings();
		printf(
	...
	/**
	 * Render the Notification Title field.
	 */
	public function render_field_title(): void {
		$s = WC_Gotify_Core::instance()->get_settings();
		printf(
	...
	/**
	 * Render the Notification Message textarea.
	 */
	...
	/**
	 * Render the Async Delivery checkbox field.
	 */
	public function render_field_async(): void {
		$s         = WC_Gotify_Core::instance()->get_settings();
	...
	/**
	 * Render the Basic Auth Username field.
	 */
	public function render_field_auth_user(): void {
		$s = WC_Gotify_Core::instance()->get_settings();
		printf(
	...
	/**
	 * Render the Basic Auth Password field.
	 */
	public function render_field_auth_pass(): void {
		$s = WC_Gotify_Core::instance()->get_settings();
		printf(
	 ...
	 */
	public function handle_test_message(): void {
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
		...
		if ( is_wp_error( $result ) ) {
			/* translators: %s: error message from Gotify. */
		...
		$redirect = wp_get_referer() ?: admin_url( 'admin.php?page=wc-gotify' );
